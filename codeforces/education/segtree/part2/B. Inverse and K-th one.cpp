...
struct Lznode {
    int updated = 0; // Node x; assert(x.apply(LzNode()) == x)
    bool is_null = true;
    ...
    static Lznode composition(Lznode first, Lznode second) {
        ...
        return Lznode{
            (first.updated + second.updated) % 2,
            false
        };
    }
    static Lznode v(int updated) {
        return Lznode {
            updated,
            false
        };
...
struct Node {
    ...
    void apply(Lznode lz) {
        ...
        if (lz.updated) val = size - val; // DÒNG QUAN TRỌNG NHẤT.
        // dòng này thể hiện rằng apply lznode vào 1 range (1 node to chứ ko phải apply vào 1 node đơn lẻ)
        // apply vào 1 node đơn lẻ thì val = 1 - val. Nhưng apply vào 1 node to đùng
        // thì val = size - val. Do hàm merge() là hàm cộng nên 1 node to đùng (1 range) sẽ có giá trị size
    }
