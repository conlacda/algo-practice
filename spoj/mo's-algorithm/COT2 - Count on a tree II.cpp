...
public:
    ...
    void solve() {
        ...
        for (auto query: queries) {
            while (cur_l > query.l) cur_l--, add_or_remove(cur_l);
            while (cur_r < query.r) cur_r++, add_or_remove(cur_r);
            while (cur_l < query.l) add_or_remove(cur_l), cur_l++;
            while (cur_r > query.r) add_or_remove(cur_r), cur_r--;
            // Xét tới lca.lca(u, v)
